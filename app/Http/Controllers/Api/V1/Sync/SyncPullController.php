    ...
    private function toSalePayload(Sale $s, ?Contact $linkedContact = null): array
    {
        return [
            ...
            'lines' => $s->lines->map(fn ($l) => [
                ...
                'price' => (float) $l->price,
                'priceBase' => $l->price_base !== null ? (float) $l->price_base : null,
                'amount' => (float) $l->amount,
                'subTotal' => (float) $l->sub_total,
                'subTotalBase' => $l->sub_total_base !== null ? (float) $l->sub_total_base : null,
                'currency' => $l->currency,
                'exchangeRateFromBase' => $l->exchange_rate_from_base !== null ? (float) $l->exchange_rate_from_base : null,
                'unitOfMeasureId' => $l->unit_of_measure_id,
                'unitOfMeasurement' => $l->unit_of_measurement,
            ])->all(),
            'inventoryConsumption' => $s->inventory_consumption ?? [],
            'dateTime' => $s->transaction_at?->toIso8601String(),
            'total' => (float) $s->total,
            'totalBase' => $s->total_base !== null ? (float) $s->total_base : null,
            'status' => $s->status,
            'currency' => $s->currency,
            'exchangeRateFromBase' => $s->exchange_rate_from_base !== null ? (float) $s->exchange_rate_from_base : null,
